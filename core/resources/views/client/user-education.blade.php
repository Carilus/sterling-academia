                    ...
                    <div class="row">
                        <div class="col-12 col-lg-8 m-auto">
                            <form class="multisteps-form__form" method="POST"
                                action="{{ route('saveUserEducation') }}">
                                @csrf
                                <!--single form panel-->
                                <div class="multisteps-form__panel shadow p-4 rounded bg-white js-active"
                                    data-animation="scaleIn">
                                    <h3 class="multisteps-form__title">Education Details</h3>
                                    <label for="educationLevel"></label>
                                    <select name="educationLevel" required class="form-control" name="educationLevel"
                                        id="educationLevel">
                                        <option disabled value="educationLevel" selected>Education Level
                                        </option>
                                        @foreach ($educationLevels as $educationLevel)
                                            <option value="{{ $educationLevel->id }}">{{ $educationLevel->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="text" name="institutionName" placeholder="Institution"
                                        class="mt-2 form-control" id="institutionName" />
                                    <input type="text" name="educationMajor" placeholder="Field"
                                        class="form-control mt-2" id="educationMajor" />
